...
class ShopifyClient {
	...
	public function getAccessToken($code) {
        ...
        public function getOrders($lastId=null){
            return $this->callPrivateAppJson("orders",$lastId);
        }
        public function getProducts($lastId=null){
            return $this->callPrivateAppJson("products",$lastId);
        }
        public function callPrivateAppJson($request,$lastId=null){
            $params = $lastId==null?"":"&since_id=".$lastId;
            $url = "https://{$this->api_key}:{$this->secret}@{$this->shop_domain}/admin/{$request}.json?limit=250".$params;
            $payload = "";
            $page=1;
            $cnt = 0;
            $result = array($request=>array());
            while($cnt==250 || $page==1){
                $response = $this->curlHttpApiRequest('GET', $url."&page={$page}", '', $payload, array());
                $response = json_decode($response, true);
                $result[$request] = array_merge($result[$request],$response[$request]);
                $cnt = count($response[$request]);
                $page++;
            }
            return $result;
        }
